...
function layer:__init(input_dim, hidden_dim)
  ...
  self.gradInput = {self.grad_c0, self.grad_h0, self.grad_x}
    ...
    -- RS EDIT: stashable h0, c0, output, cell
  self.h0_stack = {}
  self.c0_stack = {}
  self.output_stack = {}
  self.cell_stack = {}
...
-- RS EDIT: adding these functions here
function layer:pushStates()
  table.insert(self.h0_stack, self.h0:clone())
  table.insert(self.c0_stack, self.c0:clone())
  table.insert(self.output_stack, self.output:clone())
  table.insert(self.cell_stack, self.cell:clone())
end


function layer:popStates()
  self.h0 = table.remove(self.h0_stack)
  self.c0 = table.remove(self.c0_stack)
  self.output = table.remove(self.output_stack)
  self.cell = table.remove(self.cell_stack)
end
-- /RS EDIT
...
function layer:updateOutput(input)
  ...
  -- RS THEORY: want this ONLY to depend on h0, c0, and x: nothing else
  ...
  if not c0 then
    c0 = self.c0 -- RS: this is where we pick up from current state...
    ...
    elseif self.remember_states then
      ...
      c0:copy(self.cell[{{}, prev_T}]) -- isn't cell getting "injected" here? -- NOTE <-- WEIRD
    end
  end
  if not h0 then
    h0 = self.h0 -- RS: this is where we pick up from current state...
    ...
    elseif self.remember_states then
      ...
      h0:copy(self.output[{{}, prev_T}]) -- NOTE <-- WEIRD
    end
  end
  ...
  h:resize(N, T, H):zero() -- self.output gets zeroed here!
  c:resize(N, T, H):zero()
  local prev_h, prev_c = h0, c0 -- RS: note the h0 and c0 being referenced here
  ...
  for t = 1, T do
    ...
    prev_h, prev_c = next_h, next_c -- RS: this is where self.h0 and self.c0 get updated
  end
