...
local layer, parent = torch.class('nn.LSTM_no_memory', 'nn.Module')
...
function layer:updateOutput(input)
  ...
  Wh:fill(0) -- no memory weight
  bias_expand[{{}, {H + 1, 2 * H}}]:fill(0) -- no memory bias
  ...
  for t = 1, T do
    ...
    next_h:cmul(i, g)
    prev_c:fill(0) -- no previous state
